...
class Boss(Enemy):
    ...
    def align_rect_to_hitbox(self):
        """Neo rect vào hitbox theo pixel không trong suốt thực tế,
        tránh lệch khi frame có padding không đối xứng (vd lúc lật ảnh)."""
        content = self.image.get_bounding_rect()
        if content.width == 0:
            self.rect = self.image.get_rect(midbottom=self.hitbox.midbottom)
            return
        self.rect = self.image.get_rect()
        self.rect.x = self.hitbox.centerx - content.centerx
        self.rect.y = self.hitbox.bottom - content.bottom
